...
class TestPersianNumbers(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        print(f"✅ Loaded {len(cls.expected_results)} expected results from {LUT_FILENAME}")
    ...
    def _load_lookup_table(cls, file_path: str) -> None:
        ...
        for line in lines:
            if ':' in line and line.strip():
                ...
                if (line.strip().startswith('PERSIAN_NUMBERS_AS_WORDS') or 
                    line.strip() in ['{', '}']):
                    continue
    ...
    def test_all_numbers_1_to_1000(self) -> None:
        ...
        if failures:
            ...
            for i, failure in enumerate(failures[:10]):
                print(f"  {failure['number']}: expected '{failure['expected']}', "
            ...
            with self.subTest(number=number):
                ...
                if number == 0:
                    ...
                    self.assertIsInstance(actual, str, 
                                        f"Function should return a string for {number}")
                    ...
                    self.assertEqual(actual, expected_from_lut, 
    ...
    def test_performance(self) -> None:
        ...
        print(f"\n⚡ Performance Test:")
        print(f"   Total time for {PERFORMANCE_TEST_ITERATIONS} conversions: {duration:.4f} seconds")
        ...
        for number in test_cases:
            with self.subTest(number=number):
                ...
                if abs(number) > SUPPORTED_RANGE[1]:
                    error_keywords = ['خطا', 'error', 'محدوده', 'range', 'پشتیبانی', 'support']
                    self.assertTrue(
...
def test_function() -> None:
    ...
    test_cases = [
        ...
        1000, 2000, 5000,
        # Compound thousands
        1001, 1234, 2500, 3456, 9999,
        # Large numbers within supported range
        50000, 100000, 500000, 999999,
        # Current counter value
        173,
        # Some interesting numbers
        42, 123, 555, 777, 1357, 2468
    ]
    ...
    try:
        ...
        if choice == "1":
            ...
            if unit_test_success:
                ...
                print("\n" + "-" * 50)
                try:
                    run_perf = input("Run additional performance test? (y/n): ").strip().lower()
