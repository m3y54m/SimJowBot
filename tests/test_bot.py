...
import bot
from bot import (
    Config, FileManager, TwitterClient, DateTimeUtil, TwitterUtil, main
)
import tweepy
...
class TestConfig(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.original_env = {}
        for key in ['API_KEY', 'API_KEY_SECRET', 'ACCESS_TOKEN', 'ACCESS_TOKEN_SECRET', 'BEARER_TOKEN']:
...
class TestFileManager(unittest.TestCase):
    ...
    def test_read_counter_file_exists(self):
        ...
        result = self.file_manager.read_counter()
...
class TestTwitterClient(unittest.TestCase):
    ...
    def test_get_user_tweets_rate_limit(self, mock_logger):
        ...
        self.mock_client.get_users_tweets.side_effect = tweepy.TooManyRequests(mock_response)
    ...
    def test_get_user_tweets_api_error(self, mock_logger):
        ...
        self.mock_client.get_users_tweets.side_effect = tweepy.TweepyException("API Error")
    ...
    def test_post_quote_tweet_success(self, mock_get_url, mock_logger):
        ...
        result = self.twitter_client.post_quote_tweet("Test text", "123456789", "testuser")
    ...
    def test_post_quote_tweet_rate_limit(self, mock_logger):
        ...
        self.mock_client.create_tweet.side_effect = tweepy.TooManyRequests(mock_response)
    ...
    @patch('bot.logger')
    def test_try_posting_tweet_success(self, mock_logger, mock_generate_text, 
                                       mock_get_url, mock_print_info):
        ...
        self.twitter_client.post_quote_tweet = Mock(return_value="https://x.com/testuser/status/new_tweet")
...
class TestMainFunction(unittest.TestCase):
    ...
    @patch('bot.logger')
    def test_main_no_tweet_needed(self, mock_logger, mock_get_counter, 
                                  mock_file_manager, mock_twitter_client):
        """Test main function when no tweet is needed."""
        # Mock file manager
        mock_fm_instance = Mock()
        mock_fm_instance.read_counter.return_value = 100
        ...
        mock_logger.info.assert_any_call("✅ No tweet needed today. Stored counter is up to date.")
    ...
    @patch('bot.logger')
    def test_main_tweet_needed_success(self, mock_logger, mock_is_ci, mock_get_counter, 
                                       mock_file_manager, mock_twitter_client):
        """Test main function when tweet is needed and succeeds."""
        # Mock file manager
        mock_fm_instance = Mock()
        mock_fm_instance.read_counter.return_value = 98
    ...
    @patch('bot.DateTimeUtil.is_ci_environment')
    @patch('bot.logger')
    def test_main_rate_limit_in_ci(self, mock_logger, mock_is_ci, mock_get_counter, 
                                   mock_file_manager, mock_twitter_client):
        ...
        mock_logger.error.assert_any_call("❌ Rate limit active. Exiting in CI environment to allow scheduled retry.")
    ...
    @patch('bot.DateTimeUtil.is_ci_environment')
    @patch('bot.logger')
    def test_main_rate_limit_wait(self, mock_logger, mock_is_ci, mock_get_counter, 
                                  mock_file_manager, mock_twitter_client, mock_sleep):
        """Test main function waits for rate limit in non-CI environment."""
        # Mock file manager
        mock_fm_instance = Mock()
        mock_fm_instance.read_counter.return_value = 99
...
class TestErrorHandling(unittest.TestCase):
    ...
    def test_main_with_keyboard_interrupt(self, mock_logger):
        ...
        with patch('bot.DateTimeUtil.get_counter_value_for_today', side_effect=KeyboardInterrupt):
    ...
    def test_main_with_unexpected_error(self, mock_logger):
        ...
        with patch('bot.DateTimeUtil.get_counter_value_for_today', side_effect=Exception("Unexpected error")):
